...
const getBraintreeGateway = (timeout: number = BRAINTREE_TIMEOUT_PAYMENT) => {
	if (!process.env.BRAINTREE_MERCHANT_ID || !process.env.BRAINTREE_PUBLIC_KEY || !process.env.BRAINTREE_PRIVATE_KEY) {
		throw new CustomError(500, "Internal Server Error - braintree config is missing");
	}
	const gateway = new braintree.BraintreeGateway({
		environment: process.env.BRAINTREE_ENVIRONMENT === "Production" ? Environment.Production : Environment.Sandbox,
...
function logResponse(requestName: string, response: ValidatedResponse<any> | Customer | Array<Transaction>) {
...
export const transactionSale = async (request: TransactionRequest) => {
	...
	if (!response.success) {
		const prefix = ["soft_declined", "hard_declined"].includes(response?.transaction?.processorResponseType)
...
export const submitForSettlement = async (transactionId: string, amount?: string) => {
	const gateway = getBraintreeGateway();
	const response = amount
		? await gateway.transaction.submitForPartialSettlement(transactionId, amount)
...
export const createCustomer = async (request: CustomerCreateRequest): Promise<Customer> => {
	const gateway = getBraintreeGateway(BRAINTREE_TIMEOUT_CUSTOMER);
...
export const createPaymentMethod = async (request: PaymentMethodCreateRequest): Promise<PaymentMethod> => {
	const gateway = getBraintreeGateway(BRAINTREE_TIMEOUT_CUSTOMER);
...
export const findTransaction = async (orderId: string) => {
	...
	if (transactions.length === 0) {
		throw new CustomError(500, `could not find transaction with orderId ${orderId}`);
	}
...
function streamToTransaction(stream: Stream): Promise<Array<Transaction>> {
	...
	return new Promise((resolve, reject) => {
		stream.on("data", (transaction: Transaction) => transactions.push(transaction));
...
export const updatePayment = async (paymentMethodToken: string, updateRequest: PaymentMethodUpdateRequest) => {
	const gateway = getBraintreeGateway(BRAINTREE_TIMEOUT_CUSTOMER);
	const response = await gateway.paymentMethod.update(paymentMethodToken, updateRequest);
...
// TODO replace the below with our own errors
type ErrorItem = {
